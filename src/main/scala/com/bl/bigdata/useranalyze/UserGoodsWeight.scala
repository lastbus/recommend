...
class UserGoodsWeight extends Tool {
  ...
  override def run(args: Array[String]): Unit = {
    ...
    logger.info("user goods weight 开始计算........")
    ...
    trainRDD.cache()
    // 每个类别，用户浏览的最大次数
    val categoryMap = trainRDD.map { case (cookie, category, brand, rating) => ((cookie, category), 1L) }
                              .reduceByKey(_ + _)
                              .map(s => (s._1._2, s._2))
                              .reduceByKey(Math.max).collectAsMap()
    // 每个品牌，用户浏览的最大次数
    val brandMap = trainRDD.map(s => ((s._1, s._2, s._3), 1L))
                           .reduceByKey(_ + _)
    ...
    // 每个用户浏览不同品牌的次数的权重
    val userGoodsCount = trainRDD.map(s => ((s._1, s._2, s._3), s._4)).reduceByKey(_ + _)
    // 计算类别权重
    ...
    trainRDD.unpersist()
    logger.info("user goods weight 计算结束。")
  }
