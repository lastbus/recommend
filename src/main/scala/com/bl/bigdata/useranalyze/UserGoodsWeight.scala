...
class UserGoodsWeight extends Tool
{
  ...
  val decay = ConfigurationBL.get("interest.decay").toBoolean //是否考虑兴趣衰减
  ...
  override def run(args: Array[String]): Unit =
  {
    ...
    val sql = "select u.member_id, g.category_id, g.category_name, g.brand_sid, u.behavior_type, u.dt   " +
    ...
    trainRDD.unpersist()
    Message.addMessage(s"用户数： $userAccumulator")
  ...
  {
    s match {
      case "1000" => 1 case "2000" => 3 case "3000" => 0 case "4000" => 5
    }
  }

  /** 用户行为评分，乘以衰减系数 */
  def getRating2(behavior: String, date: String, now: Long): Double =
  {
    ...
    pow * Math.pow(decayCoefficient, delt)
  }
