...
object CategoryBand {
  ...
  def main(args: Array[String]) {
    Message.addMessage("================  search category brand socre begin  ================")
    val options = try {
    ...
    if (options(CategoryScoreConf.mail).toBoolean) Message.sendMail
...
object CategoryScoreConf {
  ...
  myCommandLine.addOption("hive1", to, true, " search category brand score hive table", "recommendation.ser_cate_brd_score")
  myCommandLine.addOption("hive2", to2, true, "search category score hive table", "recommendation.ser_cate_score")
  ...
  myCommandLine.addOption("m", mail, true, "send mail", "true")
