...
class AlsWithNewId {
  ...
  def userRating(day:Int): Unit = {
    val sc = SparkFactory.getSparkContext("AlsWithNewId")
    ...
    }.filter(x=>(x._1.toLong>0)&&(x._1.toLong<Int.MaxValue)&&(x._2._1.toInt>0) &&(x._2._1.toInt<Int.MaxValue))

    //userRdd.saveAsTextFile("/home/hdfs/userRddxxx")
    ...
    var cnt = 1
    val prdRdd = trainRDD.map(r=>r.user).collect()
    val upRdd = prdRdd.foreach { uId => {
      cnt+=1
      val K = 60
      val topKRecs = model.recommendProducts(uId, K)
      if(topKRecs != null) {
        val prods = topKRecs.map(r => {r.product}).mkString("#")
        val mId = uId+100000000000000L
        println(uId+"------")
        println(mId + ":" + prods)
        println(cnt + "-------------------------")
