  ...
  class AlsBasedRcmd {
    ...
    def userRating(day:Int): Unit = {
      val sc = SparkFactory.getSparkContext("ALS")
      ...
      val sql = "select member_id, event_date, behavior_type, category_sid, goods_sid from recommendation.user_behavior_raw_data  " +
        s"where dt >= $start and behavior_type=1000 and length(member_id)=15 and member_id is not null and  member_id <>'null' and member_id <>'NULL' and member_id <>''"
...
//      }
      ...
      val userRdd =  prefRdd.map{case(memberId,(goods_sid,degree))=>
        ((memberId,goods_sid),degree)
      }.reduceByKey(_ + _).map { case ((memberId, goods_sid), degree) =>
        (memberId,(goods_sid,degree))
      }.filter(x=>(x._1.toLong>100000000000000L)&&(x._2._1.toInt>0) &&(x._2._1.toInt<Int.MaxValue))

      val idTransRdd = userRdd.map { case (uId, (pId, rate)) => uId }.distinct().zipWithUniqueId()
     //val idTransRdd = userRdd.map { case (uId, (pId, rate)) => (uId,(uId.toLong -100000000000000L).toInt) }.distinct()
      ...
      val upRdd = users.foreach { case (uId, mId) => {
        ...
        if(topKRecs != null) {
          println(uId + "--"+ mId)
          val prods = topKRecs.map(r => {r.product}).mkString("#")
          println(mId + ":" + prods)
          println(cnt + "-------------------------")
